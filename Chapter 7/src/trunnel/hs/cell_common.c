...
int
trn_cell_extension_field_set_field_len(trn_cell_extension_field_t *inp, uint64_t val)
{
  inp->field_len = val;
  return 0;
}
...
trn_cell_extension_field_encode(uint8_t *output, const size_t avail, const trn_cell_extension_field_t *obj)
{
  ...
  /* Encode u64 field_len */
  trunnel_assert(written <= avail);
  if (avail - written < 8)
    goto truncated;
  trunnel_set_uint64(ptr, trunnel_htonll(obj->field_len));
  written += 8; ptr += 8;
...
trn_cell_extension_field_parse_into(trn_cell_extension_field_t *obj, const uint8_t *input, const size_t len_in)
{
  ...
  obj->field_len = trunnel_ntohll(trunnel_get_uint64(ptr));
  remaining -= 8; ptr += 8;
