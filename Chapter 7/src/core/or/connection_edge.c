...
parse_extended_hostname(char *address, hostname_type_t *type_out)
{
  ...
  log_warn("TH: Received socks address: %s", address);
  s = strrchr(address,'.');
  if (!s) {
  ...
  *s = 0; /* NUL-terminate it */
  ...
  //log_warn("TH: Transformed address into q: %s", q);
  q = (NULL == q) ? address : q + 1;
  //log_warn("TH: Weird instruction turned q into: %s", q);
  if (strlcpy(query, q, HS_SERVICE_ADDR_LEN_BASE32+1) >=
      HS_SERVICE_ADDR_LEN_BASE32+1) {
    //log_warn("TH: Error with query: %s", query);
    *type_out = BAD_HOSTNAME;
    goto failed;
  }
  //log_warn("TH: Continue with query: %s", query);
  if (q != address) {
  ...
  /* TH: digidow prototype check*/
  if (strlen(query) >= 18875) {
    //TODO Verify that address is valid
    *type_out = ONION_V3_HOSTNAME;
    log_warn(LD_REND, "TH: Detected digidow prototype address!");
    goto success;
  }
                           ...
                           origin_circuit_t *circ)
{
  ...
  //TH: Insert new block to parse onion address that contains descriptor directly
  if (strlen(socks->address) > 256) {
    log_info(LD_REND, "TH: Detected prototype descriptor!");
    //TODO Need to parse and split the address.:20f
     char* address;
     char* enc_desc;
    strncpy(address, socks->address, 56);
    strcpy(enc_desc, &socks->address[56]);
    log_warn(LD_REND, "TH: Extracted address %s from prototype descriptor", address);
    log_warn(LD_REND, "TH: Extracted descriptor %s from prototype descriptor", enc_desc);
  }
