                         ...
                         trn_cell_extension_t *private_extensions)
{
  ...
  trn_cell_introduce_encrypted_set_extensions(enc_cell, private_extensions);
                         ...
                         const hs_service_t *service)
{
  ...
  trn_cell_extension_t *extensions= trn_cell_introduce_encrypted_get_extensions(enc_cell);
  ...
  if (count > 0){
    ...
    for(size_t i=0; i<count; i++) {
      trn_cell_extension_field_t *field = trn_cell_extension_get_fields(extensions, i);
      uint8_t type=trn_cell_extension_field_get_field_type(field);
      uint64_t length=trn_cell_extension_field_get_field_len(field);
      char content[length+1];
      for(size_t j=0; j<length; j++) {
        char c = (char) trn_cell_extension_field_get_field(field, j);
        content[j]=c;
      }
      content[length]='\0';
      log_debug(LD_GENERAL, "THDEBUG: Detected Extension with Type %i and %lu bytes of data: %s", type, length, content);
      switch(type){
        case 31:
          condition=calloc(length+1, sizeof(char));
          strcpy(condition, content);
          break;
        case 32:
        ...
        case 34:
          ...
          break; //TODO: Implement logic to load descriptor if descriptor is present. Need to verify size restrictions first
        case 35:
          for (int j=0;j<32;j++){
        ...
        case 36:
          ...
          contains_descriptor=1;
          break;
        default: log_warn(LD_GENERAL, "TH: Detected unknown Extension with Type %i and %lu bytes of data: %s", type, length, content);
      }
    }
    //TH:Put the included descriptor in the hs_cache, if there is any
    if(contains_descriptor>0){
      log_info(LD_GENERAL, "Received HSSUB with callback-descriptor, building descriptor now");
      hs_descriptor_t *descriptor=tor_calloc(1, sizeof(hs_descriptor_t));
      hs_cache_client_descriptor_t *desc_entry=tor_calloc(1,sizeof(hs_cache_client_descriptor_t));

      desc_entry->encoded_desc="MINIMAL_DESCRIPTOR";
      desc_entry->expiration_ts=(time_t) duration;
      hs_parse_address(callback, &desc_entry->key, NULL, NULL);
      descriptor->plaintext_data.lifetime_sec=3600;
      descriptor->subcredential=subcred;
      hs_desc_intro_point_t *ip = hs_desc_intro_point_new();
      ip->auth_key_cert = tor_calloc(1, sizeof(tor_cert_t));
      ip->auth_key_cert->signed_key=auth_pubkey;
      smartlist_t *ip_link_specifiers=smartlist_new();
      ed25519_public_key_t node_id;
      SMARTLIST_FOREACH_BEGIN(data->link_specifiers, 
                              const link_specifier_t *, ls) {
        link_specifier_t* ls_intro=link_specifier_dup(ls);
        if (link_specifier_get_ls_type(ls_intro) == 3) {
            memcpy(node_id.pubkey,
            link_specifier_getconstarray_un_ed25519_id(ls_intro),
            ED25519_PUBKEY_LEN);
        }
        smartlist_add(ip_link_specifiers, ls_intro);
      } SMARTLIST_FOREACH_END(ls);
      ip->link_specifiers=ip_link_specifiers;
      ip->enc_key=enc_key;
      const node_t *ip_node = node_get_by_ed25519_id(&node_id);
      ip->onion_key= *node_get_curve25519_onion_key(ip_node);
      smartlist_t *intro_points=smartlist_new();
      smartlist_add(intro_points, ip);
      descriptor->encrypted_data.intro_points=intro_points;
      desc_entry->desc=descriptor;
      store_v3_desc_as_client(desc_entry);
    }
    log_info(LD_GENERAL, "Descriptor was stored successfully");
    //TH: Now fire the descriptor event, if possible
...
/** Build an INTRODUCE1 cell with arbitrary extensions from the given data.
 * The encoded cell is put in cell_out which must be of at least size 
 * RELAY_PAYLOAD_SIZE. On success, the encoded length is returned else a 
 * negative value and the content of cell_out should be ignored. */
ssize_t
hs_cell_build_extended_introduce1(const hs_cell_introduce1_data_t *data,
                         ...
                         uint8_t *cell_out)
{
  ...
  /* Set extension data.*/
  //TODO: This assumes that extensions is not empty, should probably be checked
  trn_cell_introduce1_set_extensions(cell, public_extensions);
