                         ...
                         hs_service_descriptor_t *desc)
{
  ...
  /* TH: Check if the descriptor will be published. If it is not, trigger a 
  * HS_DESC_CONTENT event to enable users to use the service. */
  if (!get_options()->PublishHidServDescriptors) {
    char *logged_desc = NULL;
    char base64_blinded_pk[ED25519_BASE64_LEN + 1];
    service_encode_descriptor(service, desc, &desc->signing_kp, &logged_desc);
    ed25519_public_to_base64(base64_blinded_pk,&desc->desc->plaintext_data.blinded_pubkey);
    control_event_hs_descriptor_content(service->onion_address,base64_blinded_pk, NULL,logged_desc);
  }
