...
static const char *posthsdescriptor_keywords[] = {
  "address", NULL,
};

static const control_cmd_syntax_t posthsdescriptor_syntax = {
  .max_args = 0,
  .accept_keywords = true,
  .allowed_keywords = posthsdescriptor_keywords,
  .want_cmddata = true,
};

/**
 * Called when we receive a POSTHSDESCRIPTOR event. Accepts same arguments 
 * as POSTDESCRIPTOR. 
 * 
 */
static int
handle_control_posthsdescriptor(control_connection_t *conn,
                                const control_cmd_args_t *cmd_args) {
  
  log_warn(LD_GENERAL, "Received newly created control port event for loading a hidden service descriptor with data %s", cmd_args->cmddata);
  const config_line_t *line;
  line = config_line_find_case(cmd_args->kwargs, "address");
  if (line) {
    log_warn(LD_GENERAL, "Successfully received encoded onion address argument: %s", line->value);

  }else {
    //TODO Should the function also support loading descriptors for directory purposes?
    control_write_endreply(conn, 400, "No address provided, cannot decrypt descriptor");
    return -1;
  }
  
  //TODO2: Need to convert that onion address in the right format. 
  ed25519_public_key_t identity_pk;
  hs_parse_address(line->value, &identity_pk, NULL, NULL);

  int status = hs_cache_store_as_client(cmd_args->cmddata, &identity_pk);
  log_warn(LD_GENERAL, "The attempt to load the descriptor led to: %i", status);
  if(status<0){
    control_write_endreply(conn, 400, "Failed to parse provided descriptor");
    return 1;
  }else {
    send_control_done(conn); 
    return 0;
...
static const control_cmd_syntax_t launch_by_extend_syntax = {
  .max_args=0,
  .accept_keywords= true,
  .allowed_keywords = launch_by_extend_keywords,
  .want_cmddata = false,
};

static int handle_control_launch_by_extend(control_connection_t *conn,
                                      const control_cmd_args_t *cmd_args){
  
  const config_line_t *arg;
  for (arg = cmd_args->kwargs; arg; arg = arg->next) {
    log_info(LD_GENERAL, "THDEBUG: Received keyword argument with key %s and value %s", arg->key, arg->value);
  }
  const config_line_t *destination=config_line_find_case(cmd_args->kwargs, "onionaddress");
  if (destination){
    ...
    log_info(LD_GENERAL, "Flags have been set up to %i", flags);
    //Setting up the Purpose
    uint8_t new_circ_purpose = CIRCUIT_PURPOSE_C_INTRODUCING;
    ed25519_public_key_t identity_key;
    hs_parse_address(destination->value, &identity_key, NULL, NULL);
    hs_ident_edge_conn_t *hs_ident= hs_ident_edge_conn_new(&identity_key);
    edge_connection_t edge_conn;
    edge_conn.hs_ident = hs_ident;
    extend_info_t *extend_info = hs_client_get_random_intro_from_edge(&edge_conn);
    
    if(!extend_info){
      log_info(LD_GENERAL, "Failed to get introduction point");
    }
    log_info(LD_GENERAL, "Ready to launch introduction circuit");
    origin_circuit_t *circ = circuit_launch_by_extend_info(new_circ_purpose, extend_info, flags);
    circ->hs_ident=hs_ident_circuit_new(&identity_key);
    log_info(LD_GENERAL, "intro circuit (%i) has been launched", circ->global_identifier);
    control_printf_midreply(conn, 250, "%i", circ->global_identifier);
    send_control_done(conn);
    return 0;
  }else{
    control_write_endreply(conn, 400, "No valid destination onion address was provided");
    return 1;
  }
}
...
static void 
cell_extension_set_field_to_string(trn_cell_extension_field_t *inp, const char* value, uint8_t type){
  size_t string_length;
  string_length=strlen(value);
  trn_cell_extension_field_set_field_type(inp, type);
  trn_cell_extension_field_set_field_len(inp, (uint64_t) string_length);
  trn_cell_extension_field_setlen_field(inp, string_length);
...
static void
cell_extension_set_field_to_bytes(trn_cell_extension_field_t *inp, uint64_t length, const uint8_t* data, uint8_t type){
  trn_cell_extension_field_set_field_type(inp, type);
...
static const control_cmd_syntax_t posthssub_syntax = {
  .max_args = 0,
  .accept_keywords = true,
...
/**
 * Called when we receive a POSTHSSUB request
 * Accepts two arguments: 
 * "publisher" -> The onion address of the service to subscribe to
                         ...
                         const control_cmd_args_t *cmd_args)
{
  ...
  for (arg = cmd_args->kwargs; arg; arg = arg->next) {
    log_info(LD_GENERAL, "Received keyword argument with key %s and value %s", arg->key, arg->value);
  }
  ...
  log_info(LD_GENERAL, "Received command to sub at %s requesting event info for '%s' to: %s", publisher->value, condition->value, callback->value);
  ...
  if (circ == NULL || circ->hs_ident == NULL) {
    ...
    control_write_endreply(conn, 400, "Error: No HS_IDENT set on provided circuit. Cannot send HSSUB");
    return -1;
  }
  if (circ!=NULL && circ->base_.state == CIRCUIT_STATE_OPEN){
    ...
    if (intro_point == NULL) {
      control_write_endreply(conn, 400, "Could not find out which IntroPoint we are using");
      return 1;
    }
    ...
    ed25519_public_key_t callback_public_key;
    hs_descriptor_t *callback_descriptor=NULL;
    if (hs_parse_address(callback->value, &callback_public_key, NULL, NULL) != 0){
      log_err(LD_GENERAL, "Could not parse provided callback address. Aborting HSPOST");
      control_write_endreply(conn, 400, "Error: Your provided callback address could not be parsed");
      return -1;
    }
    if(cmd_args->cmddata_len > 0) {
      log_debug(LD_GENERAL, "We received a callback descriptor, try to pack descriptor in INTRODUCE1");
      
      //OK, we have a valid onion address as callback
      hs_client_decode_descriptor(cmd_args->cmddata, &callback_public_key, &callback_descriptor);
      if (callback_descriptor == NULL){
        control_write_endreply(conn, 400, "Error: Your provided descriptor data could not be parsed");
        return -1;
      }
    } else{
      log_debug(LD_GENERAL, "We received a callback address without descriptor. Checking if descriptor is in cache:");
      callback_descriptor=(hs_descriptor_t*)hs_cache_lookup_as_client(&callback_public_key);
    }

    if (callback_descriptor) {
      hs_desc_intro_point_t* ip=NULL;    
      ip = (hs_desc_intro_point_t*) smartlist_get(callback_descriptor->encrypted_data.intro_points, 0);

      if(ip == NULL){
        control_write_endreply(conn, 400, "Error: You specified a service descriptor without introduction points");
        return -1;
      }
      ed25519_public_key_t sign_key= ip->auth_key_cert->signed_key;
      hs_subcredential_t subc = callback_descriptor->subcredential;
      intro1_data.link_specifiers=ip->link_specifiers;
      intro1_data.client_kp=&rend_pubkey;
      //TODO I have 20 bytes of data left I can encode in there
      intro1_data.rendezvous_cookie=rend_cookie;
      
      //Set up extensions that only get added in this case:
      trn_cell_extension_set_num(private_extensions, 6);
      trn_cell_extension_setlen_fields(private_extensions, 6);
      trn_cell_extension_field_t *callback_subcredential = trn_cell_extension_field_new();
      trn_cell_extension_field_t *callback_authkey = trn_cell_extension_field_new();
      trn_cell_extension_field_t *callback_enc_key = trn_cell_extension_field_new();
      cell_extension_set_field_to_bytes(callback_subcredential, 32, subc.subcred, 34);
      cell_extension_set_field_to_bytes(callback_authkey, 32, sign_key.pubkey, 35);
      cell_extension_set_field_to_bytes(callback_enc_key, 32, ip->enc_key.public_key, 36);
      trn_cell_extension_set_fields(private_extensions, 3, callback_subcredential);
      trn_cell_extension_set_fields(private_extensions, 4, callback_authkey);
      trn_cell_extension_set_fields(private_extensions, 5, callback_enc_key);
    ...
    if(duration){
      ...
      log_debug(LD_GENERAL, "Expiry deadline is: %li + %li", time(NULL), 7200l);
      time_t expiry_time=time(NULL) + (time_t)7200;
      log_debug(LD_GENERAL, "Expiry deadline is: %li + %li = %li", time(NULL), 7200l, expiry_time);
      snprintf(default_duration,(size_t) 22, "%li", time(NULL) + 7200l);
    ...
    ssize_t length=hs_cell_build_extended_introduce1(&intro1_data, public_extensions, private_extensions, cell_out);

    if(length<0){
      log_info(LD_GENERAL, "TH: Error, failed to build extended introduce1 cell!");
      control_write_endreply(conn, 400, "Error: Error occurred when building extended introduce1 cell");
      return -1;
    }else{
      log_info(LD_GENERAL, "TH: Sending introduce cell of size %li (of %i available)",length, RELAY_PAYLOAD_SIZE);
      int returnCode=relay_send_command_from_edge(CONTROL_CELL_ID, TO_CIRCUIT(circ),
                                   RELAY_COMMAND_INTRODUCE1, 
                                   (const char*) cell_out, length, circ->cpath->prev);
      if (returnCode) {
        log_err(LD_GENERAL, "TH: Unable to send introduce cell on circuit %u", TO_CIRCUIT(circ)->n_circ_id);
        control_write_endreply(conn, 400, "Error: Could not send introduce cell on provided circuit");
        return -1;
      } else{
        circuit_change_purpose(TO_CIRCUIT(circ), CIRCUIT_PURPOSE_C_INTRODUCE_ACK_WAIT);
      }
    }
    ...
    control_write_endreply(conn, 400, "Circuit is not yet ready for connection");
    return -1;
  }
}
